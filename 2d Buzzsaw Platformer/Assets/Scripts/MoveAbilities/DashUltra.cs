...
public class DashUltra : MoveAbility
{

    public float dashSpeed;
    public float dashPeriod;
    public float verticalDashMultiplier;
    public float dashAngleBuffer;
    ...
    void DashInDirection(float horizontalDirection, float verticalDirection)
    {
        ...
        horizontalDirection = (Mathf.Abs(verticalDirection) > Mathf.Abs(horizontalDirection) && Mathf.Abs(horizontalDirection) < dashAngleBuffer) ? 0 : Mathf.Sign(horizontalDirection);
        verticalDirection = (Mathf.Abs(horizontalDirection) > Mathf.Abs(verticalDirection) && Mathf.Abs(verticalDirection) < dashAngleBuffer) ? 0 : Mathf.Sign(verticalDirection);
        player.rb.velocity = (Vector2.right * dashSpeed * horizontalDirection) + (Vector2.up * dashSpeed * verticalDashMultiplier * verticalDirection);
